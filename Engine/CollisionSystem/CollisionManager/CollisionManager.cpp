...
#include "CollisionSystem/ColliderShapes/ColliderShapeBox2D/ColliderShapeBox2D.h"
#include "CollisionSystem/ColliderShapes/ColliderShapeMapChip2D/ColliderShapeMapChip2D.h"
#include "calc.h"
#include <functional>
							...
							if (velocity.y < 0) {
								...
								if (beforePos.x <= chipPos.x - mapChipScale.x - scale.x) {
									if (beforePos.y <= chipPos.y + mapChipScale.y + scale.y) {
										...
										if (Calc::Outer({ (chipPos.x - mapChipScale.x) - (beforePos.x + scale.x), (chipPos.y + mapChipScale.y) - (beforePos.y - scale.y) },
